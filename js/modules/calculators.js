...
function calculateFoundationMaterials(inputs, areas) {
  ...
  const cement = Math.ceil(
    (dryVolume * mix.cement) / (mix.sum * cementBagVolume),
  );
...
function calculatePlinthBeamMaterials(inputs, areas) {
  ...
  const beamDepth = 0.3; // 300 mm = 0.30 m
...
function calculateBrickworkMaterials(inputs, areas) {
  ...
  const mortarWetVolume = totalWallVolume * 0.3;
  ...
  const cement = Math.ceil(
    (mortarDryVolume * ratio.cement) / (ratio.sum * cementBagVolume),
  );
...
function calculatePlasterMaterials(inputs, areas) {
  ...
  const internalPlasterVolume =
    areas.netInternalWallArea * 2 * internalPlasterThicknessM;
  ...
  const externalPlasterVolume =
    areas.netExternalWallArea * 2 * externalPlasterThicknessM;
  ...
  const cement = Math.ceil(
    (dryVolume * ratio.cement) / (ratio.sum * cementBagVolume),
  );
